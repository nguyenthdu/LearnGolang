...
type flt1 func(int) bool
type slice_split func([]int) ([]int, []int)

func isOdd1(integer int) bool { // check if integer is odd
	if integer%2 == 0 {
		return false
	}
...
func filter_factory(f flt1) slice_split { // split the slice on basis of func
...
func main() {
	s := []int{1, 2, 3, 4, 5, 7}
	fmt.Println("s = ", s)
	odd_even_function := filter_factory(isOdd1)
